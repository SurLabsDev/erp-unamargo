...
9. Nada específico de un cliente hardcodeado: "Unamargo" solo puede aparecer en `config/instance.json` (no versionado), en el seed demo y en los **scripts de carga inicial de una sola vez** (`scripts/import-catalogo.ts` y `scripts/extract-demo-catalog.ts`, junto a sus datos en `scripts/data/`), que por definición son de una instancia concreta: cargan el catálogo real de ese cliente y no los ejecuta la app. Esa excepción está declarada en `DECISIONS.md` (20/08) y no se extiende a `src/`.
